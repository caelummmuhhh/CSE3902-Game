...
namespace MainGame.Rooms
{
    public static class RoomFactory
    {
        ...
        public static IRoom GenerateRoom(string roomFile, IPlayer player, AudioManager audioManager)
        {
            ...
            {
                ParseItemsAndBlocks(ref lines[i], room, i - 2, audioManager);
                ParseEnemies(lines[i], room, player, i - 2, audioManager);
            }

            GenerateHitboxes(room, lines[0].Replace(",", ""));
            AddAdditionalElements(room, lines[0].Replace(",", ""), audioManager);

            return room;
        }
        private static void AddAdditionalElements(IRoom room, string roomType, AudioManager audioManager)
        {
            ...
            else if (roomType.Equals("dungeonOldMan"))
            {
                ...
                room.RoomItems.Add(
                            new GenericItem(
                                    new Vector2(72 * Constants.UniversalScale, 4 * Constants.BlockSize + Constants.HudAndMenuHeight),
                                    SpriteFactory.CreateItemSprite((ItemTypes)Enum.Parse(typeof(ItemTypes), "Fire")),
                                    (ItemTypes)Enum.Parse(typeof(ItemTypes), "Fire"),
                                    audioManager
                                    ));
                room.RoomItems.Add(
                            new GenericItem(
                                    new Vector2(168 * Constants.UniversalScale, 4 * Constants.BlockSize + Constants.HudAndMenuHeight),
                                    SpriteFactory.CreateItemSprite((ItemTypes)Enum.Parse(typeof(ItemTypes), "Fire")),
                                    (ItemTypes)Enum.Parse(typeof(ItemTypes), "Fire"),
                                    audioManager
                                    ));
        ...
        private static void ParseItemsAndBlocks(ref string line, IRoom room, int yOffset, AudioManager audioManager)
        {
                    ...
                    if (itemSuccess)
                    {
                        room.RoomItems.Add(
                            new GenericItem(
                                    ...
                                    SpriteFactory.CreateItemSprite((ItemTypes)item), (ItemTypes)item, audioManager
                                    ));
                        objects[i] = "-";
                    }
                }
        ...
        private static void ParseEnemies(string line, IRoom room, IPlayer player, int yOffset, AudioManager audioManager)
        {
            ...
            for (int i = 0; i < objects.Length; i++)
            {
                ...
                {
                    IEnemy createdEnemy = EnemyUtils.CreateItemBindedEnemy(objects[i], spawnPosition, out IItem createdItem, player, audioManager);
